  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-3 pt-5 border-t-2 border-white/10">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <a
              href={`discord://-/users/${equipo.discord_id_capitan}`}
              onClick={(e) => e.stopPropagation()}
              className="flex-1 px-5 py-3 font-display font-bold text-base uppercase tracking-wider border-4 border-black neo-shadow transition-transform duration-100 hover:scale-105 active:scale-95 flex items-center justify-center gap-2 bg-[#5865F2] text-white rounded-xl"
            >
            ...
            {/* R6 Tracker link for the team (using first roster member or captain) */}
            {equipo.integrantes_ubisoft_ids && equipo.integrantes_ubisoft_ids.length > 0 && (
              <a
                href={`https://r6.tracker.network/r6siege/profile/${equipo.plataforma === "pc" ? "uplay" : equipo.plataforma === "playstation" ? "psn" : "xbl"}/${encodeURIComponent(equipo.integrantes_ubisoft_ids[0])}/overview`}
                target="_blank"
                rel="noopener noreferrer"
                onClick={(e) => e.stopPropagation()}
                className="flex-1 px-5 py-3 font-display font-bold text-base uppercase tracking-wider border-4 border-black neo-shadow transition-transform duration-100 hover:scale-105 active:scale-95 flex items-center justify-center gap-2 bg-[#1c0f2f] text-white border-white rounded-xl"
              >
